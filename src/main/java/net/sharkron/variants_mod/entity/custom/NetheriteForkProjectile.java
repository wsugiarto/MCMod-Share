...
public class NetheriteForkProjectile extends AbstractHurtingProjectile{
    private int life;
    private int pierces;

    public NetheriteForkProjectile(EntityType<? extends NetheriteForkProjectile> p, Level level){
        super(p, level);
    }

    // This should be constructor for non abstract
    public NetheriteForkProjectile(Level level, LivingEntity owner, double x, double y, double z){
        this(ModEntity.NETHERITE_FORK_PROJECTILE.get(), level);
        this.setOwner(owner);
    ...
    // Special properties for Wither Skull
